...
export class SorobanTransactionSimulationService {
  async simulate(transactionXdr: string): Promise<SorobanTransactionSimulation> {
    ...
    return {
      ...
      readEntries: asNonNegativeNumber(resources.readEntries()),
      writeEntries: asNonNegativeNumber(resources.writeEntries()),
      readBytes: asString(resources.readBytes()),
      writeBytes: asString(resources.writeBytes()),
      requiredBaseFee,
      priorityFees: {
